...
class GCNModel(torch.nn.Module): #one which takes in the number of layers
    def __init__(self, input_dim, number_of_classes, layers = 3, hidden_layer=16, layer_name='GCNLayer', dp_rate=0):
        """GNNModel.
        ...
        self.gcn_layers = {'GCNLayer': GCNLayer, 'GCN': GCNConv}
    ...
    def forward(self, x, A):
      for conv in self.model_layers:
        if isinstance(conv, [GCNLayer, GCNConv]):
          x = conv(x, A)
        else:
          x = conv(x)
